...
//			for(xscan = ROI_left+1; xscan <= ROI_right; xscan++){
	...
	public void findPetiole(ImageProcessor ip) {
		...
		if (verbose > 0) IJ.log("start findPetiole");
...
//		topLine = widths.getTopLine();
		...
		if (verbose > 1) IJ.log("top: " + IJ.d2s(top,0));
		ip.drawLine((int) topLine[0],(int) topLine[1],(int) topLine[2],(int) topLine[3]);
		if (verbose > 1) IJ.log("top line " +  IJ.d2s(topLine[0]) +" "+
			IJ.d2s(topLine[1]) +" "+
			IJ.d2s(topLine[2]) +" "+
			IJ.d2s(topLine[3]));
...
public void addPetioleToManager(ImagePlus imp, ImageProcessor ip, RoiManager rm,int leaf){
	...
	if (verbose > 0) IJ.log("start addToManager");
	...
	if (verbose > 1) IJ.log("length of array: " + IJ.d2s(petiole_x.length) + " nPoints: " + IJ.d2s(nPoints,0) + " step: " + IJ.d2s(step));
	for(int i = 0; i < nPoints; i++) {
		roi_x[i] = (int) Math.round(petiole_x[(int) Math.round(i*step)]);
		roi_y[i] = (int) Math.round(petiole_y[(int) Math.round(i*step)]);
		if (verbose > 1) IJ.log("i: " + IJ.d2s(i,0) 
			+ " i*step: " + IJ.d2s(i*step) 
			+ " rounded: " + IJ.d2s(Math.round(i*step))
			+ " x:" + IJ.d2s(roi_x[i])
			+ " y:" + IJ.d2s(roi_y[i]));
	} 
